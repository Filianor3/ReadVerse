...
public class UserDAOTest extends TestCase {
    ...
    public void testSetAndGetUserAvatar() throws SQLException {
        UserDAO ud = new DbUserDAO(true);
        int userId = 1;
        int avatarId = 123;

        // Set user avatar
        ud.setUserAvatar(userId, avatarId);

        // Get user avatar
        int retrievedAvatarId = ud.getUserAvatar(userId);

        assertEquals(avatarId, retrievedAvatarId);
    }


    public void testGetAllAvatars() throws SQLException {
        UserDAO ud = new DbUserDAO(true);
        List<String> expectedAvatarPaths = new ArrayList<>();
        // Add expected avatar paths to the expectedAvatarPaths list

        List<String> actualAvatarPaths = ud.getAllAvatars();

        assertEquals(expectedAvatarPaths, actualAvatarPaths);
    }

    public void testGetAvatarPath() throws SQLException {
        UserDAO ud = new DbUserDAO(true);
        int avatarId = 123;
        String actualAvatarPath = ud.getAvatarPath(avatarId);
        assertEquals(null, actualAvatarPath);
    }

    public void testGetUserByName() throws SQLException {
        UserDAO ud = new DbUserDAO(true);
        String username = "testuser";
        User actualUser = ud.getUserByName(username);
